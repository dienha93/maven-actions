...
class InputValidator {
  ...
  validateOperation(errors) {
    ...
    if (/^\s+$/.test(operation)) {
      errors.push(
        `- operation: Invalid value '${operation}'. Must be one of: ${this.validOperations.join(', ')}`
      );
      return;
    }
    ...
    if (operation.includes(',')) {
      errors.push(
        '- operation: Multiple operations not supported. Please specify only one operation.'
      );
      return;
    }
    ...
    if (!this.validOperations.includes(trimmedOperation)) {
      ...
      errors.push(
        `- operation: Invalid value '${operation}'. Must be one of: ${this.validOperations.join(', ')}${suggestions ? '. ' + suggestions : ''}`
      );
    }
  }
  ...
  validateJavaVersion(errors) {
    ...
    if (javaVersion && !this.validJavaVersions.includes(javaVersion)) {
      errors.push(
        `- java-version: Invalid value '${javaVersion}'. Must be one of: ${this.validJavaVersions.join(', ')}`
      );
    }
  }
  ...
  validateJavaDistribution(errors) {
    ...
    if (javaDistribution && !this.validJavaDistributions.includes(javaDistribution)) {
      errors.push(
        `- java-distribution: Invalid value '${javaDistribution}'. Must be one of: ${this.validJavaDistributions.join(', ')}`
      );
    }
  }
  ...
  validateMavenVersion(errors) {
    ...
    if (mavenVersion) {
      ...
      if (!versionRegex.test(mavenVersion)) {
        errors.push(
          `- maven-version: Invalid format '${mavenVersion}'. Must be in format X.Y.Z (e.g., 3.9.5)`
        );
      }
    }
  }
  ...
  validateWorkingDirectory(errors) {
    ...
    if (workingDir) {
      ...
      if (workingDir.length > this.maxPathLength) {
        errors.push(
          `- working-directory: Path too long (${workingDir.length} > ${this.maxPathLength})`
        );
      }
  ...
  validateSettingsFile(errors) {
    ...
    if (settingsFile) {
      ...
      if (settingsFile.length > this.maxPathLength) {
        errors.push(
          `- settings-file: Path too long (${settingsFile.length} > ${this.maxPathLength})`
        );
      }
    }
  }
  ...
  validateMavenArgs(errors) {
    ...
    if (mavenArgs) {
      ...
      const dangerousArgs = [
        '--settings',
        '-s', // Settings should use settings-file input
        '--file',
        '-f', // POM file changes not allowed
        '--batch-mode',
        '-B', // We control batch mode
        '--quiet',
        '-q', // We control logging
        '--debug',
        '-X' // We control debug mode
      ];
  ...
  validateBooleanInputs(errors) {
    const booleanInputs = ['cache-enabled', 'force-install'];
